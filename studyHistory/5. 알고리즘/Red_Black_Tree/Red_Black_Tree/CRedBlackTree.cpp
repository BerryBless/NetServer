...
bool CRedBlackTree::InsertNode(long long Data) {
	// 지역변수
	stNODE *pNode;	// 새로운 노드
...
bool CRedBlackTree::DeleteNode(long long Data) {
	stNODE *pNode = Find(Data); // 데이터를 가지고 있는 노드를 찾음
	...
	if (pNode->pLeft != _Nil &&
		...
		pNode->Data = pDelNode->Data;			// 나보다 작은것중 가장큰게 내가 됨
	}
...
CRedBlackTree::stNODE *CRedBlackTree::Find(long long Data) {
	stNODE *pNode = _root;
	while (pNode != _Nil) {
		if (pNode->Data == Data) {
			return pNode;
...
void CRedBlackTree::Rotate_Left(long long Data) {
	stNODE *pNode = _root;
...
void CRedBlackTree::Rotate_Right(long long Data) {
	stNODE *pNode = _root;
