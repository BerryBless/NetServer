...
CDisplayMap::CDisplayMap() {
	...
	_hPathPen = (HPEN) CreatePen(PS_SOLID, 3, RGB(0,255,255));
	...
	_hOpenBrush = (HBRUSH) CreateSolidBrush(RGB(248, 231, 239));	// 예약지점
	_hCloseBrush = (HBRUSH) CreateSolidBrush(RGB(231, 248, 240));	// 완료지점
...
void CDisplayMap::DrawRect(int i, int j) {
	...
	case (short)eMAP::Wall:// 벽
		SelectObject(_hMemDC, _hWallBrush);
		break;
	case (short)eMAP::Start://시작지점
		SelectObject(_hMemDC, _hStartBrush);
		break;
	case (short)eMAP::End://끝지점
		SelectObject(_hMemDC, _hEndBrush);
		break;
	case (short)eMAP::Open:// 예약지점
		SelectObject(_hMemDC, _hOpenBrush);
		break;
	case (short)eMAP::Close://방문지점
		SelectObject(_hMemDC, _hCloseBrush);
		break;
	case (short) eMAP::MAX + 1:
		SelectObject(_hMemDC, _hCloseBrush);
		break;
	default:// 비어있음
	...
	if(pPathNode->pParent != NULL)
		Drawfrom(rect.left + _half, rect.top + _half,
			pPathNode->pParent->X - pPathNode->X, pPathNode->pParent->Y - pPathNode->Y,
			_half -1);

}

void CDisplayMap::DrawPath(CList<stPOS> &path) {
	...
	++iter;
	SelectObject(_hMemDC, _hPathPen);		// 사각형 팬
