#include "CLanServer.h"

// cmd code
#define dfEXIT_CODE 0xFFFFFFFF // GQCS()에서 이게 오면 종료

// CS
#define SESSION_LOCK(pSession)		SessionLock(pSession)
#define SESSION_UNLOCK(pSession)	SessionUnlock(pSession)
#define SESSION_CONTAINER_LOCK()	SessionContainerLock()
#define SESSION_CONTAINER_UNLOCK()	SessionContainerUnlock()

#define dfMIN(a,b) a>b?b:a
#define dfMAX(a,b) a<b?b:a

// TEMP 전역변수
CObjectPool<CPacket> g_packetPool;

CLanServer::CLanServer() {
	// 정말 1초로 카운팅
	//timeBeginPeriod(1);
	// 모니터링 초기화
	ZeroMemory(&_monitor, sizeof(MONITOR));
	// 세션 컨테이너 락 초기화
	InitializeSRWLock(&_sessionContainerLock);
}

CLanServer::~CLanServer() {
	WSACleanup();
}

bool CLanServer::Start(u_long IP, u_short prot, BYTE workerThreadCount, BYTE maxRunThreadCount, BOOL nagle, u_short maxConnection) {
	// 이미 실행중?
	if (_isRunning == true) {
		_LOG(dfLOG_LEVEL_ERROR, L"Network is already running\n");
		return false;
	}
	_isRunning = false;
	// 서버 정보 넣기
	_Port = prot;
	_bindIP = IP;
	_workerThreadCount = workerThreadCount;
	_maxRunThreadCount = dfMIN(workerThreadCount, maxRunThreadCount);
	_isNagle = nagle;
	_maxConnection = maxConnection;

	// 정보로 리슨 만들기
	if (CreateListenSocket() == false) {
		// 리슨 소켓 생성 실패
		return false;
	}

	// 스레드 핸들 생성
	_NumThreads = _workerThreadCount + 2; // _workerThreadCount + acceptThread + monitorThread
	_hThreads = new HANDLE[(long long) _NumThreads]; // 배열 생성

	// 스레드 실행
	BeginThreads();

	// 서버 실행 완료
	_isRunning = true;
	return _isRunning;
}

bool CLanServer::Start(wchar_t *wsConfigPath) {
	// TODO 파일 패치로 파싱해서 실행
	return Start(INADDR_ANY, 10101, 24,12,true,3000);
}

bool CLanServer::Stop() {
	// TODO 일시정지
	return false;
}

void CLanServer::Quit() {
	// 서버 종료
	PostQueuedCompletionStatus(_hIOCP, dfEXIT_CODE, dfEXIT_CODE, NULL); // 워커 하나 종료하라고 알려줌
	_isRunning = false; // 종료!
	closesocket(_listensock); // accept 스레드 강제종료
}

void CLanServer::WaitForThreadsFin() {
	// 서버 종료 기다리기
	// TODO 에러체크
	DWORD retval = WaitForMultipleObjects(_NumThreads, _hThreads, TRUE, INFINITE);
	switch (retval) {
	case WAIT_FAILED:
		break;
	case WAIT_TIMEOUT:
		break;
	default:
		break;
	}
}

int CLanServer::GetSessionCount() {
	// 현재 연결된 세션
	int count;
	SESSION_CONTAINER_LOCK();
	count = _sessionContainer.size();
	SESSION_CONTAINER_UNLOCK();
	return count;
}

bool CLanServer::Disconnect(SESSION_ID SessionID) {
	// TODO 세션 끊기?
	SESSION *pSession = FindSession(SessionID);
	if (pSession == NULL) {
		_LOG(dfLOG_LEVEL_ERROR, L"//Disconnect ERROR :: can not find session..");
		OnError(12345, L"Disconnect ERROR :: can not find session..");
		return false;
	}
	pSession->_isAlive = false;

	//DeleteSessionData(SessionID);
	return true;
}

bool CLanServer::SendPacket(SESSION_ID SessionID, CPacket *pPacket) {
	// TODO errorcode, logic 넘버링
	SESSION *pSession = FindSession(SessionID);
	if (pSession == NULL) {
		_LOG(dfLOG_LEVEL_ERROR, L"//SendPacket ERROR :: can not find session..");
		OnError(12345, L"SendPacket ERROR :: can not find session..");
		return false;
	}
	// 지워진(끊어진) 세션
	if (pSession->_isAlive == false) {
		_LOG(dfLOG_LEVEL_ERROR, L"//SendPacket ERROR :: Session is colsed..");
		OnError(12345, L"SendPacket ERROR :: Session is colsed..");
		return false;
	}

	// 페킷 포인터를 센드큐에
	pSession->_sendQueue.Enqueue((char *) &pPacket, sizeof(CPacket *));
	
	// monitor
	InterlockedIncrement(&_monitor._sendPacketCalc);
	return SendPost(pSession, 12345);
}

bool CLanServer::CreateListenSocket() {
	WSADATA wsa;

	// 윈속 초기화
	if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
		_LOG(dfLOG_LEVEL_ERROR, L"\n////// WSAStartup() errcode[%d]\n", WSAGetLastError());
		return false;
	}
	_LOG(dfLOG_LEVEL_ERROR, L"WSAStartup OK..\n");

	// socket()
	_listensock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (_listensock == INVALID_SOCKET) {
		_LOG(dfLOG_LEVEL_ERROR, L"\n////// socket() errcode[%d]\n", WSAGetLastError());
		return FALSE;
	}
	_LOG(dfLOG_LEVEL_ERROR, L"socket() OK ..\n");

	// bind()
	SOCKADDR_IN addr;
	addr.sin_family = AF_INET;
	addr.sin_port = htons(_Port);
	addr.sin_addr.S_un.S_addr = htonl(_bindIP);

	int bindRet = bind(_listensock, (SOCKADDR *) &addr, sizeof(addr));
	if (bindRet == SOCKET_ERROR) {
		_LOG(dfLOG_LEVEL_ERROR, L"\n////// bind() errcode[%d]\n", WSAGetLastError());
		closesocket(_listensock);
		return false;
	}
	_LOG(dfLOG_LEVEL_ERROR, L"bind() OK ..\n");

	// listen()
	int listenRet = listen(_listensock, SOMAXCONN);
	if (listenRet == SOCKET_ERROR) {
		_LOG(dfLOG_LEVEL_ERROR, L"\n////// listen() errcode[%d]\n", WSAGetLastError());
		closesocket(_listensock);
		return false;
	}
	_LOG(dfLOG_LEVEL_ERROR, L"listen() OK ..\n");


	// TODO 코어 갯수 확인?
	// 사용자 문제인가? 라이브러리 문제인가?

	// CreateIoCompletionPort()
	_hIOCP = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, _maxRunThreadCount);
	if (_hIOCP == NULL) {
		_LOG(dfLOG_LEVEL_ERROR, L"\n////// CreateIoCompletionPort() errcode[%d]\n", WSAGetLastError());
		closesocket(_listensock);
		return false;
	}
	_LOG(dfLOG_LEVEL_ERROR, L"CreateIoCompletionPort() OK ..\n");
	return true;
}

void CLanServer::BeginThreads() {
	// n : _workerThreadCount
	// 0~n-1 : worker
	// n : accept
	// n+1 : Monitor
	int i = 0;

	// 워커스레드 만들기
	for (; i < _workerThreadCount; i++) {
		// 스레드는 static이라 this로 오브젝트 알려줌..
		_hThreads[i] = (HANDLE) _beginthreadex(nullptr, 0, WorkerThread, this, 0, nullptr);

	}
	//  accept 스레드
	_hThreads[i] = (HANDLE) _beginthreadex(nullptr, 0, AcceptThread, this, 0, nullptr);
	++i;
	// 모니터링 스레드
	_hThreads[i] = (HANDLE) _beginthreadex(nullptr, 0, MonitorThread, this, 0, nullptr);
}

unsigned int __stdcall CLanServer::WorkerThread(LPVOID arg) {
	CLanServer *pServer = (CLanServer *) arg;
	while (true) {
		// static 이니까.. 오브젝트 내에서 실행하게 만들어주기
		if (pServer->OnGQCS() == false) {
			break;
		}
	}
	Sleep(1);
	_LOG(dfLOG_LEVEL_ERROR, L"-- Worker Thread IS Closed..\n");
	return 0;
}

unsigned int __stdcall CLanServer::AcceptThread(LPVOID arg) {
	CLanServer *pServer = (CLanServer *) arg;
	while (true) {
		// static 이니까.. 오브젝트 내에서 실행하게 만들어주기
		if (pServer->AcceptProc() == false) {
			break;
		}
	}
	Sleep(1);
	_LOG(dfLOG_LEVEL_ERROR, L"-- Accept Thread IS Closed..\n");
	return 0;
}

unsigned int __stdcall CLanServer::MonitorThread(LPVOID arg) {
	CLanServer *pServer = (CLanServer *) arg;
	while (true) {
		// static 이니까.. 오브젝트 내에서 실행하게 만들어주기
		if (pServer->NetMonitorProc() == false) {
			break;
		}
	}
	Sleep(1);
	_LOG(dfLOG_LEVEL_ERROR, L"-- Monitor Thread IS Closed..\n");
	return 0;
}

bool CLanServer::OnGQCS() {
	DWORD transferredSize = 0;
	SESSION_ID completionKey = 0;
	WSAOVERLAPPED *pOverlapped = nullptr;
	SESSION *pSession = nullptr;

	// GQCS()
	BOOL GQCSRet = GetQueuedCompletionStatus(_hIOCP, &transferredSize, (PULONG_PTR) &completionKey, &pOverlapped, INFINITE);

	// 완료통지 확인
	if (pOverlapped == NULL) {
		// 스레드 종료 
		if (transferredSize == dfEXIT_CODE && (long long) completionKey == dfEXIT_CODE) {
			PostQueuedCompletionStatus(_hIOCP, dfEXIT_CODE, dfEXIT_CODE, NULL); // 워커 하나 종료하라고 알려줌
			return false;
		}
		_LOG(dfLOG_LEVEL_ERROR, L"overlapped is NULL ERROR CODE [%d]", WSAGetLastError());
		OnError(dfLOGIC_FROM_WORKER + 1, L"IOCP ERROR :: overlapped is NULL");
		return true;
	}

	// 세션의 로직 처리
	pSession = FindSession(completionKey);

	SESSION_LOCK(pSession);
	do {
		if (pSession->_ID == 0) {
			// 이미 지워진 세션
			_LOG(dfLOG_LEVEL_NOTICE, L"Deleted Session\n");
			break;
		}
		if (transferredSize > 0 && GQCSRet == TRUE) {
			if (pOverlapped == &pSession->_recvOverlapped) {
				// WSARecv가 완료됨
				RecvProc(pSession, transferredSize);
			}
			if (pOverlapped == &pSession->_sendOverlapped) {
				// WSASend가 완료됨
				SendProc(pSession, transferredSize);
			}
		}
	} while (0);
	SESSION_UNLOCK(pSession);
	DecrementIOCount(pSession, dfLOGIC_FROM_WORKER + 100);

	return true;
}

bool CLanServer::SendProc(SESSION *pSession, DWORD transferredSize) {
	if (pSession == NULL) {
		CRASH();
	}
	// 완료통지 온 패킷 지우기
	CPacket *pPacketBufs[100];
	int deqsize = pSession->_sendedPacketCnt * sizeof(CPacket *);
	int pekret = pSession->_sendQueue.Dequeue((char *) pPacketBufs, deqsize);
	if (pekret != deqsize){
		CRASH();
	}
	for (int i = 0; i < pSession->_sendedPacketCnt; ++i) {
		// TODO 참조카운터 감소
		//delete pPacketBufs[i];
		g_packetPool.Free(pPacketBufs[i]);
	}
	pSession->_sendedPacketCnt = 0;

	// isSens = false
	//InterlockedExchange8((CHAR *) &pSession->_isSend, FALSE);
	pSession->_isSend = false;
	

	// 보낼것이 남아있음 sendpost
	SendPost(pSession, dfLOGIC_FROM_CPMPLETE_SEND + 100);
	return true;
}

bool CLanServer::RecvProc(SESSION *pSession, DWORD transferredSize) {
	// recv신호가 옴

	// 온만큼 링버퍼 Rear빼주기
	int movRet = pSession->_recvQueue.MoveRear(transferredSize);
	if (transferredSize != movRet) {
		_LOG(dfLOG_LEVEL_ERROR, L" ID[%lld] :: transferredSize[%d] != movRet[%d]", pSession->_ID, transferredSize, movRet);
		CRASH();
	}
	DWORD msgByte = 0; // 메시지 처리

	// return val
	int headerPeekRet;
	int headerMoveRet;
	int payloadDeqRet;
	USHORT header;

	while (msgByte < transferredSize) {
		if (pSession->_isAlive == false) {
			return false;
		}
		//CPacket *pPacket = new CPacket;  // TODO 풀로 받아오기
		CPacket *pPacket = g_packetPool.Alloc();  // TODO 풀로 받아오기
		if (pPacket == NULL) {
			CRASH();
		}
		pPacket->Clear();
		// 모니터링 체크
		InterlockedIncrement(&_monitor._recvPacketCalc);

		if (pSession->_recvQueue.GetUseSize() <= sizeof(USHORT)) 			{
			// 헤더보다 적음
			break;
		}

		// 헤더는 읽을 수 있음
		headerPeekRet =  pSession->_recvQueue.Peek((char *) &header, sizeof(USHORT));
		if (headerPeekRet != sizeof(USHORT)) {
			// 무결성 검사
			CRASH();
		}

		if (pSession->_recvQueue.GetUseSize() < (int) (sizeof(USHORT) + header)) 			{
			// 패킷 전체크기보다 적음
			break;
		}

		// 온전한 패킷이 온걸 확인
		// 이미 알고있는 정보는 넘어가기
		headerMoveRet = pSession->_recvQueue.MoveFront(sizeof(USHORT));
		if (headerMoveRet != sizeof(USHORT)) {
			// 무결성 검사
			CRASH();
		}
		
		// 페이로드 크기를 알았으니 헤더는 제 역할을 다함
		// 페이로드만 페킷에 넣어주기
		payloadDeqRet = pSession->_recvQueue.Dequeue(pPacket->GetWritePtr(), (int) header);
		if (payloadDeqRet != header) {
			// 무결성 검사
			CRASH();
		}
		pPacket->MoveWritePos(payloadDeqRet);
		OnRecv(pSession->_ID, pPacket);

		msgByte += (payloadDeqRet + sizeof(USHORT));

	}

	// RecvPost
	return RecvPost(pSession, dfLOGIC_FROM_CPMPLETE_RECV + 200);
}

bool CLanServer::AcceptProc() {
	// Accept처리
	SOCKADDR_IN clientaddr;
	int addrlen = sizeof(clientaddr);
	SOCKET clientsock;
	HANDLE hResult;

	// 동기
	clientsock = accept(_listensock, (SOCKADDR *) &clientaddr, &addrlen);
	if (clientsock == INVALID_SOCKET) {
		int err = WSAGetLastError();
		if (err == WSAENOTSOCK || err == WSAEINTR) {
			// 리슨소켓 닫음
			// 어셉트 스레드 종료
			return false;
		}
		_LOG(dfLOG_LEVEL_ERROR, L"Socket Accept [Error: %d]", err);
		OnError(err, L"Socket Accept");
		return _isRunning;
	}
	// 컨텐츠에서 처리할것 블랙,화이트 리스트, 지역차단 등등
	if (OnConnectionRequest(clientaddr.sin_addr.S_un.S_addr, clientaddr.sin_port) == false) {
		// 차단된 IP대역
		WCHAR IP[16] = {0,};
		InetNtop(AF_INET, &clientaddr.sin_addr.S_un.S_addr, IP, 16);
		_LOG(dfLOG_LEVEL_NOTICE, L"Socket Accept Denied IP[%s] Port[%u]\n",	IP, ntohs(clientaddr.sin_port));
		closesocket(clientsock);
	}

	// 네이글 옵션
	if (_isNagle == true) {
		BOOL optval = _isNagle;
		int optRet = setsockopt(clientsock, IPPROTO_TCP, TCP_NODELAY, (char *) &optval, sizeof(optval));
		if (optRet == SOCKET_ERROR) {
			_LOG(dfLOG_LEVEL_ERROR, L"Socketopt Nagle Error[%d]\n", WSAGetLastError());
			closesocket(clientsock);
			return false;
		}
	}

	// 세션 만들기
	SESSION *pSession = CreateSession(clientsock, clientaddr);

	// IOCP
	hResult = CreateIoCompletionPort((HANDLE) clientsock, _hIOCP, (ULONG_PTR) pSession->_ID, NULL);
	if (hResult == NULL) {
		int err = WSAGetLastError();
		_LOG(dfLOG_LEVEL_ERROR, L"////// Accept CreateIoCompletionPort() errcode[%d]", err);
		OnError(err, L"Accept CreateIoCompletionPort()");
		return false;
	}
	pSession->_isAlive = true; // 세션이 사용가능해짐

	// 컨탠츠에서 클라이언트가 완료됐을때 처리 할 가상함수
	OnClientJoin(pSession->_ID);

	// WSARecv걸어주기
	RecvPost(pSession, dfLOGIC_FROM_THREAD + 200, true);

	// 모니터링
	InterlockedIncrement(&_monitor._acceptCount);

	return _isRunning;
}

bool CLanServer::NetMonitorProc() {
	// 1초마다 0으로 초기화
	Sleep(1000);

	_monitor._sendPacketTPS = _monitor._sendPacketCalc;
	_monitor._sendPacketCalc= 0;

	_monitor._recvPacketTPS = _monitor._recvPacketCalc;
	_monitor._recvPacketCalc = 0;

	return _isRunning;
}

bool CLanServer::SendPost(SESSION *pSession, int logic) {
	if (pSession == NULL) {
		CRASH();
	}
	if (pSession->_isAlive == false) {
		return false;
	}

	BOOL isSend = InterlockedExchange8((CHAR *) &pSession->_isSend, TRUE);
	if (isSend == TRUE) {
		return FALSE;
	}
	if (pSession->_sendQueue.GetUseSize() == 0) {
		if (InterlockedExchange8((CHAR *) &pSession->_isSend, FALSE) == FALSE) {
			_LOG(dfLOG_LEVEL_ERROR, L"SendPost(%d) _isSend Exchange false to false", logic);
			CRASH();
		}
		return FALSE;
	}

	WSABUF bufferSet[100]; // 패킷 갯수만큼
	DWORD byteSends;

	SetWSABuffer(bufferSet, pSession, FALSE, logic + 1);

	// IO 카운트
	IncrementIOCount(pSession, logic + 1);

	// 오버랩 초기화
	memset(&pSession->_sendOverlapped, 0, sizeof(pSession->_sendOverlapped));
	// WSASend()
	int sendRet = WSASend(pSession->_sock, bufferSet, pSession->_sendedPacketCnt, &byteSends, 0, &pSession->_sendOverlapped, nullptr);
	if (sendRet == SOCKET_ERROR) {
		int err = WSAGetLastError();

		if (err != WSA_IO_PENDING) {
			if (err != 10054 && err != 10053)
				_LOG(dfLOG_LEVEL_ERROR, L"//// WSASend(%d) ERROR [%d]", logic, err);
			// 디스커넥트
			DecrementIOCount(pSession, logic + 1);
		}
		// 비동기로 잘들어감
		//_LOG(dfLOG_LEVEL_ERROR, L"//// WSASend WSA_IO_PENDING [%d]", err);
	}

	return true;
}

bool CLanServer::RecvPost(SESSION *pSession, int logic, bool isAccept) {
	if (pSession == NULL) {
		CRASH();
	}
	if (pSession->_isAlive == false) {
		return false;
	}
	WSABUF bufferSet[2]; // 0 : 현재, 1 : 링버퍼 처음
	DWORD flag = 0;
	DWORD byteRecvs;

	SetWSABuffer(bufferSet, pSession, TRUE, logic + 1);

	/*if (isAccept)
		bufferSet[1].len = 0;*/

	// IO 카운트
	IncrementIOCount(pSession, logic + 1);

	// 오버랩 초기화
	memset(&pSession->_recvOverlapped, 0, sizeof(pSession->_recvOverlapped));
	//WSARecv()
	int recvRet = WSARecv(pSession->_sock, bufferSet, 2, &byteRecvs, &flag, &pSession->_recvOverlapped, nullptr);
	if (recvRet == SOCKET_ERROR) {
		int err = WSAGetLastError();

		if (err != WSA_IO_PENDING) {
			if (err != 10054 && err != 10053)
				_LOG(dfLOG_LEVEL_ERROR, L"//// %d :: WSARecv ERROR [%d]\n", logic, err);
			// 디스커넥트
			DecrementIOCount(pSession, logic + 1);
			return false;
		}
	}
	// 동기로 완료
	return true;
}

bool CLanServer::SetWSABuffer(WSABUF *pBufSet, SESSION *pSession, bool isRecv, int logic) {
	if (isRecv) {
		char *pBuf = pSession->_recvQueue.GetBufferPtr();
		char *pRear = pSession->_recvQueue.GetRearBufferPtr();
		int  enSize = pSession->_recvQueue.DirectEnqueueSize();
		int  frSize = pSession->_recvQueue.GetFreeSize();
		// recv버퍼에 넣기
		pBufSet[0].buf = pRear;
		pBufSet[0].len = enSize;
		pBufSet[1].buf = pBuf;
		pBufSet[1].len = frSize - enSize;
		if (pBufSet[0].len + pBufSet[1].len > frSize) {
			_LOG(dfLOG_LEVEL_ERROR, L"SetWSABuffer(%d) :: pBufSet[0].len + pBufSet[1].len != FreeSize", logic);
			CRASH();
		}
	} else {
		// 패킷 을 버퍼에 등록
		CPacket *pPacketBufs[100];
		// 스냅
		DWORD snapSize = pSession->_sendQueue.GetUseSize() ;
		int packetCnt = snapSize / sizeof(CPacket *);
		if (packetCnt > 100) {
			snapSize = sizeof(CPacket *) * 100;
			packetCnt = 100;
		}

		// 패킷 Peek
		int pekret = pSession->_sendQueue.Peek((char *) pPacketBufs, snapSize);
		if (pekret != snapSize)
			CRASH();

		// wsabuffer에 등록
		for (int i = 0; i < packetCnt; ++i) {
			pPacketBufs[i]->SetHeader();
			pBufSet[i].buf = pPacketBufs[i]->GetSendPtr();
			pBufSet[i].len = pPacketBufs[i]->GetSendSize();
		}

		// 처리한만큼 저장
		pSession->_sendedPacketCnt = packetCnt;
	}

	return true;
}

bool CLanServer::IncrementIOCount(SESSION *pSession, int logic) {
	// ++IOCount
	if (pSession == NULL) return false;
	DWORD retval = InterlockedIncrement(&pSession->_IOcount);
	_LOG(dfLOG_LEVEL_DEBUG, L"IncrementIOCount() ID[%lld] :: logic[%d]", pSession->_ID, logic);
	return true;
}

bool CLanServer::DecrementIOCount(SESSION *pSession, int logic) {
	// --IOCount
	if (pSession == NULL) return false;
	DWORD retval = InterlockedDecrement(&pSession->_IOcount);
	_LOG(dfLOG_LEVEL_DEBUG, L"DecrementIOCount(SID[%lld]  logic[%d])", pSession->_ID, logic);

	if (retval == 0) {
		// TODO disconnect
		//Disconnect(pSession, logic + 1);
		ReleaseSession(pSession, logic + 1);
		return false;
	}
	if (retval < 0) {
		_LOG(dfLOG_LEVEL_ERROR,
			L"DecrementIOCount(%d) pSession->_IOcount [%d]\n\
SOCK[%d] :: recv [%d]byte, send [%d]byte, IOCount[%d], isSend [%d]",
logic, pSession->_IOcount, pSession->_sock, pSession->_recvQueue.GetUseSize(), pSession->_sendQueue.GetUseSize(), pSession->_IOcount, pSession->_isSend);
		CRASH();
	}
	return true;
}

bool CLanServer::ReleaseSession(SESSION *pSession, int logic) {
	//TODO
	if (pSession == NULL) {
		CRASH();
	}
	OnClientLeave(pSession->_ID);
	closesocket(pSession->_sock);

	pSession->_isAlive = false;
	DeleteSessionData(pSession->_ID);

	InterlockedIncrement(&_monitor._disconnectCount);
	return true;
}


SESSION *CLanServer::CreateSession(SOCKET sock, SOCKADDR_IN addr) {
	// ID생성
	SESSION_ID id = GenerateSessionID();

	// 할당
	//SESSION *pSession = new SESSION;
	SESSION *pSession = _sessionPool.Alloc();
	if (pSession == NULL) {
		// TODO 에러처리
		CRASH();
	}

	// 초기화
	InitializeSRWLock(&pSession->_lock);
	pSession->_isSend = false;
	pSession->_isAlive = false;
	pSession->_sendedPacketCnt = 0;

	ZeroMemory(&pSession->_recvOverlapped, sizeof(WSAOVERLAPPED));
	ZeroMemory(&pSession->_sendOverlapped, sizeof(WSAOVERLAPPED));
	pSession->_recvQueue.ClearBuffer();
	pSession->_sendQueue.ClearBuffer();

	// 정보 셋팅
	pSession->_ID = id;
	pSession->_sock = sock;
	pSession->_IP = addr.sin_addr.S_un.S_addr;
	pSession->_port = addr.sin_port;
	
	// 컨테이너에 넣기
	InsertSessionData(pSession);

	return pSession;
}

SESSION_ID CLanServer::GenerateSessionID() {
	return _IDGenerater++;
}

SESSION *CLanServer::FindSession(SESSION_ID sessionID) {
	SESSION *pSession = NULL;
	SESSION_CONTAINER_LOCK();
	do {
		// 맵에서 찾고
		auto iter = _sessionContainer.find(sessionID);
		// 없나 확인
		if (iter == _sessionContainer.end())
			break;
		// 찾았다
		pSession = iter->second;
	} while (0);
	SESSION_CONTAINER_UNLOCK();
	return pSession;
}

void CLanServer::InsertSessionData(SESSION *pSession) {
	SESSION_CONTAINER_LOCK();
	_sessionContainer.insert(std::make_pair(pSession->_ID, pSession));
	SESSION_CONTAINER_UNLOCK();
}

void CLanServer::DeleteSessionData(SESSION_ID sessionID) {
	SESSION *pSession;
	SESSION_CONTAINER_LOCK();
	auto iter = _sessionContainer.find(sessionID);
	if (iter == _sessionContainer.end())
		CRASH();

	pSession = iter->second;
	SESSION_LOCK(pSession);
	pSession->_ID = 0;
	SESSION_UNLOCK(pSession);
	_sessionContainer.erase(iter);

	SESSION_CONTAINER_UNLOCK();
	//delete pSession;
	_sessionPool.Free(pSession);
}

void CLanServer::SessionLock(SESSION *pSession) {
	AcquireSRWLockExclusive(&pSession->_lock);
}

void CLanServer::SessionUnlock(SESSION *pSession) {
	ReleaseSRWLockExclusive(&pSession->_lock);
}

void CLanServer::SessionContainerLock() {
	AcquireSRWLockExclusive(&_sessionContainerLock);
}

void CLanServer::SessionContainerUnlock() {
	ReleaseSRWLockExclusive(&_sessionContainerLock);
}
