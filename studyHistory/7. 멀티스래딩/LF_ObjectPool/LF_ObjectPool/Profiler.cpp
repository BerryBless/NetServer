...
SRWLOCK CProfiler::_proLock;
DWORD CProfiler::_tlsIdx;
CProfiler **CProfiler::_profiler;
int CProfiler::_profilerIdx = 0;
int CProfiler::_profilerCount = 0;
...
int CProfiler::TagToIndex(const wchar_t *tag) {
	...
	return dfSAMPLE_FULL; // 인덱스가 꽉참, 새로은 태그
}
...
void CProfiler::ProfileBegin(const wchar_t *tag) {
	...
	if (_samples[index].isCheack == false) {
		...
		wcscpy_s(_samples[index].Tag, _countof(_samples[index].Tag), tag);
...
void CProfiler::ProfileEnd(const wchar_t *tag) {
	...
	// 찾는 정보가 없음!
	if (_samples[index].isCheack == false || index == dfSAMPLE_FULL) {
		printf_s("\n\nPROFILE 정보가 없음!\n\n");
		CRASH();
	}
...
void CProfiler::ProfilePrintFile(const wchar_t *filename) {
	AcquireSRWLockExclusive(&_proLock);
	// 출력하기
	FILE *fp = stdout;
	...
	// 출력 부분
	wprintf_s(L"[%02d/%02d/%02d %02d:%02d:%02d] PRINT LOG : %s\n", t.tm_mon + 1, t.tm_mday, (t.tm_year + 1900) % 100,
...
----------------------------------------------------------------------------------------------------\n", _threadID);
	...
	fclose(fp);
	ReleaseSRWLockExclusive(&_proLock);
}
...
// TLS
void CProfiler::InitProfiler(int threadCnt) {
	_tlsIdx = TlsAlloc();
	InitializeSRWLock(&_proLock);
	_profilerCount = threadCnt;
	// 메모리 침범으로 부터 안전하게 분리
	_profiler = new CProfiler *[_profilerCount];

	for (int i = 0; i < _profilerCount; i++) {
		_profiler[i] = new CProfiler;
	}
