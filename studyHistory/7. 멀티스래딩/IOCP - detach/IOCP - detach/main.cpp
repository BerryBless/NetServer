...
#define dfTHREAD_NUM 7 //N-1 : WORKER  1 :Accept
...
int main() {
	...
	hThread[0] = (HANDLE) _beginthreadex(NULL, 0, NetworkAcceptThread, NULL, 0, NULL);

	for (int i = 1; i < dfTHREAD_NUM; i++) {
		hThread[i] = (HANDLE) _beginthreadex(NULL, 0, NetworkWorkerThread, NULL, 0, NULL);
	}
