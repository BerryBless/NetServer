...
void *CMemory::Alloc(int size) {
	...
	void *pRet = nullptr;
	CMemoryPool::BLOCK_NODE *pNode;
	if (allocSize > MAX_ALLOC_SIZE) {
		// 그냥 할당
		pRet = _aligned_malloc(allocSize, 8);
		pRet = (CMemoryPool::BLOCK_NODE *) pRet + 1;
	} else {
		// 관리하는 테이블에서 꺼내오기
		pRet = _poolTable[allocSize]->Alloc();
	}
	pNode = (CMemoryPool::BLOCK_NODE *) pRet - 1;
...
void CMemory::Free(void *ptr) {
	CMemoryPool::BLOCK_NODE *pNode = (CMemoryPool::BLOCK_NODE *) ptr - 1;
