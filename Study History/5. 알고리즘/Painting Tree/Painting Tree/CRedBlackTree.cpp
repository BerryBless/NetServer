...
void CRedBlackTree::SetPaint(HWND hWnd) {
	_paintManager.SetHWND(hWnd);
}
...
bool CRedBlackTree::DeleteNode(int iData) {
	...
	if (pNode->pLeft != _Nil &&
		...
		pNode->iData = pDelNode->iData;			// 나보다 작은것중 가장큰게 내가 됨
	}
...
CRedBlackTree::stNODE *CRedBlackTree::Find(int iData) {
	stNODE *pNode = _root;
	while (pNode != _Nil) {
...
// 참고
// 내가 만든거 아님
// https://studyres.com/doc/2312926/binary-search-tree-visualization-algorithm?page=4
void CRedBlackTree::DisplayTree() {
	if (_root == _Nil) return;
	// 오프셋 지정
	int oX = 0;				// 수평 오프셋
	int oY = (_hc - _d) / 2;	// 수직 오프셋

	int w;	// 트리의 현재 너비
	int rX;	// 트리의 현재 X좌표
	// 루트부터 재귀 시작
	DrawTree(_root, oX, oY, rX, w);

}
void CRedBlackTree::DisplayTree(int iWidth) {
	DrawTree(_root, 0, iWidth, _hc);
}
// oX, oY 초기 오프셋
void CRedBlackTree::DrawTree(stNODE *pNode, int oX, int oY, int &rX, int &w) {
	if (pNode == NULL)return;
	w = 0;
	int stW = 0;	// 서브트리의 너비
	int stRX;		// 하위 트리 루트의 X좌표
	...
	if (pNode->pLeft == _Nil &&
		...
		// 왼쪽 서브트리 그리기
		if (pNode->pLeft != _Nil) {
			DrawTree(pNode->pLeft, oX, oY + _hc, stRX, stW);
			w = stW;
			//oX+stW-d/2
			rX = oX + stW - _d / 2;
			// 왼쪽 서브트리와 연결하는 선
			//rX+d/2,oY+d -> stRX+d/2,oY+hc
			_paintManager.PrintLine(rX + _d / 2, oY + _d, stRX + _d / 2, oY + _hc);
		} else {
			// 왼쪽 서브트리가 없는경우
			w = _d / 2;
			rX = oX;
		...
		if (pNode->pRight != _Nil) {
			DrawTree(pNode->pRight, oX + w, oY + _hc, stRX, stW);
			w += stW;
			// 오른쪽 서브트리와 연결하는 선
			//rX+d/2,oY+d -> stRX+d/2,oY+hc
			_paintManager.PrintLine(rX + _d / 2, oY + _d, stRX + _d / 2, oY + _hc);
	...
	// 노드 정보를 출력
	_paintManager.PrintNode(rX, oY, _d, pNode->iData, pNode->Color);
}

void CRedBlackTree::DrawTree(stNODE *pNode, int sW, int eW, int oY) {
	if (pNode == _Nil) return;
	int mid = (sW + eW) / 2;
	if (pNode->pLeft != _Nil) {
		DrawTree(pNode->pLeft, sW, mid, oY + _hc);

		_paintManager.PrintLine(mid + _d / 2, oY + _d, (sW + mid) / 2 + _d / 2, oY + _hc);
	}
	if (pNode->pRight != _Nil) {
		DrawTree(pNode->pRight, mid, eW, oY + _hc);
		_paintManager.PrintLine(mid + _d / 2, oY + _d, (mid + eW) / 2 + _d / 2, oY + _hc);
	}
	_paintManager.PrintNode(mid, oY, _d, pNode->iData, pNode->Color);
}

void CRedBlackTree::Rotate_Left(int iData) {
	Rotate_Left(Find(iData));
...
void CRedBlackTree::Rotate_Left(stNODE *pPivotNode) {
	if (pPivotNode == NULL || pPivotNode == _Nil) return;
...
void CRedBlackTree::Rotate_Right(int iData) {
	Rotate_Right(Find(iData));
}
