...
private:
	...
	struct BLOCK_NODE {
		...
		alignas(8) BLOCK_NODE *pNEXT;						// 다음노드
	};
private:
	BLOCK_NODE *_pFreeNode;								// 메모리풀 스택의 top
...
inline CObjectPool<DATA>::CObjectPool(int iBlockNum, bool bPlacementNew) {
	...
	_iBlockSize = sizeof(DATA) + sizeof(BLOCK_NODE);		// 블럭 사이즈 계산
	_pFreeNode = nullptr;
	// 메모리 확보 시도
	for (int i = 0; i < iBlockNum; i++) {
		// 노드 한개
		BLOCK_NODE *pNode = (BLOCK_NODE *) malloc(_iBlockSize);
...
inline DATA *CObjectPool<DATA>::Alloc(void) {
	...
	if (_pFreeNode == nullptr) {							// 공간이 부족할때
		...
		// 
		// -----------------  --->  -----------------  --->  -----------------  ---> .... 
		// | BLOCK_NODE |        | BLOCK_NODE |        | BLOCK_NODE |
		// -----------------        -----------------        ----------------- 
		// |               |        |               |        |               |
...
inline bool CObjectPool<DATA>::Free(DATA *pData) {
	...
	BLOCK_NODE *pNode = ((BLOCK_NODE *) pData - 1);			// 노드 사이즈만큼 앞으로!
