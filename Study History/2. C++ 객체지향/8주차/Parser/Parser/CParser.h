...
private:
	...
	struct stNamespaceInfoNode
	{
		stNamespaceInfo Info;
		stNamespaceInfoNode* tail;
	};
	class CNamespaceList {
	private:
		stNamespaceInfoNode* _head;
	public:
		CNamespaceList();									// 생성자 (head = nullptr)
		~CNamespaceList();									// 소멸 : clear 호출
		void Push_back(const WCHAR * name , int start);		// 꼬리에 넣기
		stNamespaceInfo* Find(const WCHAR* name);						// 해당 네임스페이스의 Start리턴 / 못찾으면 -1(음수값은 절대 안나오기 때문)
		void Clear(stNamespaceInfoNode* node);				// 메모리 할당 해지
	};
...
private:
	...
	int _filesize;	// 파일크기
	CNamespaceList _namespaceList; // 네임스페이스 인덱스리스트
	int _iStart;		// 인덱스 시작
	int _iEnd;			// 인덱스 끝
