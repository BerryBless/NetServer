...
bool CChatServer::MonitoringProc() {
	...
	_ChatTPS = InterlockedExchange(&_ChatCalc, 0);
...
void CChatServer::PacketProc(CPacket *pPacket, SESSION_ID SessionID, WORD type) {
	...
	case CHAT_PACKET_TYPE::PACKET_CS_CHAT_REQ_MESSAGE:
		InterlockedIncrement(&_ChatCalc);
...
chat Msg TPS\t[%lld]\n\
",
monitor._acceptPerSec, monitor._sendPacketPerSec, monitor._recvPacketPerSec,_LoginTPS,_SectorMoveTPS,_ChatTPS);
	fwprintf_s(fp, L"\n\
----------------------------TOTAL-------------------------------------\n\
packet\t\t[%lld]\n\
sended Byte\t[%lld]\n\
